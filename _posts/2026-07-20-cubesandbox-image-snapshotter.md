...
| **CubeSandbox** | 双路径：OCI 分层 **或** 预构建 ext4 原始镜像 | **VM 级**（rootfs + 内存 整机快照） | **virtio-pmem (NVDIMM)** + virtio-fs | **cubecow：XFS reflink (FICLONE) 文件级 COW** |
...
CubeSandbox 的 **Cubelet**（Go）实现了 CRI 风格的镜像服务，但针对 microVM 场景做了**双路径分发**。入口在 `Cubelet/internal/cube/server/images/cube_image_pull_route.go:127` 的 `CubeImageService.PullImage`，按 `ImageSpec.StorageMedia` 注解分流。

### 3.1 路径 A：标准 OCI 镜像（containerd overlayfs）

默认 media type，Cubelet 是 containerd v2 pull+unpack 管线的薄封装。`PullRegistryImage`（`image_pull.go:98`）调用 `c.client.Pull(...)`，带：
...
### 3.2 路径 B：预构建 ext4 OS 镜像（pmem 路径，cubebox 模板用）
...
### 3.4 OCI 镜像与 template 的关系：直接启动 vs. 快克隆基底

> **结论：OCI/Docker 镜像可以直接启动 sandbox，无需先转 template；但 fast clone / 整机快照 / 原地回滚这些能力是 template 体系提供的，裸 OCI sandbox 拿不到。OCI 镜像是"内容来源"，template 是"快克隆基底"，两者互补。**

#### 3.4.1 OCI 镜像可直接启动（无 template 前置条件）

`RunCubeSandbox` 带一个 docker/OCI 镜像即可起一个可运行的 microVM。是否走"从 template 创建"路径由 `GetSnapshotTemplateID()`（`Cubelet/plugins/workflow/engine.go:160`）判定——**仅当请求带 `MasterAnnotationAppSnapshotTemplateID` 注解且 `InstanceType=cubebox` 时才返回 templateID**。不带该注解即走普通创建路径。

不带 templateID 时的存储分配（`Cubelet/storage/local.go:962-977`）：

```
hasSnapshotTemplate == false  →  dealDefaultMedium  →  CreateDefaultMediumVolume
```

`CreateDefaultMediumVolume`（`Cubelet/storage/cubecow_volume_manager.go`）：

```go
name := fmt.Sprintf("sb-%s-%s", sandboxID, volumeName)   // 注意：不是 rootfs-gen
return m.createInitializedVolume(ctx, name, sizeBytes)    // CreateVolume + mkfs.ext4
```

即可写层是一个**全新的普通 cubecow volume**（`sb-<sandboxID>-<emptyDir卷名>`），**不是**从 template FICLONE 派生的 `sb-<id>-rootfs-gen0`。此时 sandbox：

- VM boot rootfs (pmem0) = 固定 `cube-guest-image-cpu.img`；
- 容器 rootfs = `overlay(lowerdir=virtiofs(OCI 各层), upperdir=cubecow 普通卷, workdir=...)`；
- 完全可运行，无需 template。

#### 3.4.2 fast clone / 快照 / 回滚依赖 template 传承链

`CommitSandbox`/`RollbackSandbox`/`CreateSandboxRootfsFromTemplate` 围绕 **template + rootfs-gen 传承链**运作，前提是 `hasSnapshotTemplate == true`：
...
OCI 直接启动的 sandbox 可写层是 `sb-<id>-<volName>`（普通 volume），**不在 rootfs-gen 传承链里**。`CommitSandbox` 的 `selectSnapshotRootfs`（`cubecow_snapshot_artifacts.go:382`）按 `sb-<id>-rootfs-gen` 前缀找根卷——所以整机快照/回滚机制面向 template 传承链，不面向裸 OCI sandbox。

#### 3.4.3 关键细节：FICLONE 只捕获可写卷，不捕获 virtiofs 只读层

这是最易误解的点。OCI 启动的 sandbox rootfs：

```
lowerdir = virtiofs 共享的 OCI 镜像各层 (containerd overlayfs 的 snapshots/N/fs)   ← 只读，host 共享
upperdir = cubecow 可写卷 (sb-<id>-<volName>)                                       ← FICLONE 只拍这个
```

`CommitSandbox` 做的是 `FICLONE(可写卷) → tpl-<id>-rootfs`。**OCI 只读层不在快照里**——它们仍以 virtiofs 共享目录形式留在 host 上，clone 出来的新 sandbox 仍需挂同一份 OCI 层作 lowerdir。

对比 Path B（ext4）：把所有内容烘焙进**单个 ext4 镜像文件**，可写层即该镜像的 cubecow 派生卷，所以 `tpl-<id>-rootfs` 快照**自包含**，clone 出的 sandbox 不依赖额外共享层。这就是 fast clone/rollback 故事在 ext4 路径上最干净的根因。

| | Path A（docker/OCI） | Path B（ext4） |
|---|---|---|
| 可写层来源 | `CreateDefaultMediumVolume` → 普通 cubecow 卷 `sb-<id>-<volName>` | 从 template FICLONE 派生 `sb-<id>-rootfs-gen0` |
| 只读层 | virtiofs 共享 OCI 各层（host 共享，不进快照） | 烘焙进 ext4 镜像（自包含） |
| 裸启动是否可运行 | ✅ 是 | ✅ 是（但 ext4 镜像本身需先构建/下载） |
| fast clone / 整机快照 / 原地回滚 | ❌ 需先 commit 成 template | ✅ 原生（template 传承链） |

#### 3.4.4 典型工作流

1. **OCI 直接启动**：`RunCubeSandbox(image=docker://...)` 起可运行 sandbox，装好环境/代码。此时无 template，无 fast clone。
2. **提交成 template**：`CommitSandbox`/`AppSnapshot` 把运行中状态拍成 `tpl-<id>-rootfs` + `tpl-<id>-memory`。AppSnapshot 走 `dealCowTemplateBuildRootfs`，建 `tpl-<id>-build-rootfs` 作构建可写层；OCI 只读层仍以 virtiofs 提供内容。
3. **从 template 批量克隆**：`RunCubeSandbox` 带 `templateID` 注解 → `CreateSandboxRootfsFromTemplate` 一次 FICLONE 出 `sb-<id>-rootfs-gen0`，毫秒级（`examples/snapshot-rollback-clone/06_clone_n.py`、`07_clone_concurrent.py`）。
4. **运行中打快照 / 原地回滚**：基于 rootfs-gen 传承链做 `CommitSandbox`/`RollbackSandbox`。

> 一句话：**转换成 template 不是运行的必要条件，而是启用快照/克隆能力的必要条件。**
...
| **镜像形态** | OCI 分层 tar | OCI 分层 → EROFS blob | OCI 分层（复用 containerd） | **双路径**：OCI 分层（复用 containerd）**或** 预构建单文件 ext4 原始镜像（URL 下载） |
...
| **与 containerd 关系** | 即 containerd | 即 containerd | snapshotter 消费者 | **部分复用**（OCI 路径用其 pull/unpack/overlayfs）+ 自建 VM 快照子系统 |
...
3. **双存储后端按 `StorageMedia` 分发**：`""`→containerd content+overlayfs（解包进 `/data/cubelet/root`）；`"ext4"`→`ext4image.EnsurePmemFile` 下载预构建 `.ext4`+内核，containerd 完全 bypass（`PullImage` 返回 `ErrSkip`）。
