...
|Catalog Operator|CRDs, ClusterServiceVersions|

它们需管理的资源如下：
|Component|Manage Resource|
|-|-|
|OLM Operator|ClusterServiceVersions，OperatorGroup|
|Catalog Operator|InstallPlan，CatalogSource，Subscription|

### OLM Operator

当ClusterServiceVersion（CSV）中指定的需要资源已经在Cluster上部署，OLM Operator会负责部署CSV中定义的Application（Deployment，SA, RBAC等）。OLM Operator通常会Watch全部的namespace，但也可以配置为Watch指定的一个或多个namespace。
...
- 如果Operator的 CSV 与 OperatorGroup 存在于同一命名空间中，并且其 CSV 的 InstallModes 支持 OperatorGroup 所针对的命名空间集，则称该Operator是 OperatorGroup 的成员。
- CSV 必须一个 OperatorGroup 的Active Member，而且该OperatorGroup没有与其他的OperatorGroup发生API conflicts
...
### Catalog Operator
Catalog Operator负责解析和安装CSV和其指定的所需资源，它还负责Watch CatalogSource以获取频道中软件包的更新，并将它们（可选地自动）升级到最新的可用版本。

希望在Channel中跟踪Bundle 更新的用户可以创建Subcription资源，在该资源中配置所需的包、Channel和从哪个CatalogSource中获取更新。找到更新后，Catalog Manager会在Namespace中创建InstallPlan。用户也可以直接创建 InstallPlan 资源，其中包含所需 CSV 的名称和批准策略，Catalog Operator 会创建执行计划以创建所有必需的资源。获得批准后，Catalog Operator 将在 InstallPlan 中创建所有资源；OLM Operator会继续安装 CSV。
...
- 监视已解决的安装计划并为其创建所有发现的资源（如果由用户批准或自动批准）。
...
- 带有Image引用的 grpc：OLM 将拉取Image并运行一个具有 api 端点的 pod，该端点可以查询存储中的元数据。
