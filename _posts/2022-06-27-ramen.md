...
`Ramen`是一个`OCM`的placement extension，它可以跨托管集群为工作负载及其持久数据提供恢复和relocation服务。`Ramen`提供云原生接口来协调工作负载及其在PersistentVolume中的数据，包括：
- 将工作负载重新定位到对等集群，以实现跨集群的计划迁移
- 由于集群的意外丢失，将正在工作负载恢复到对等集群
...
```
DR Policy Controller的作用有：
1. 验证DRCluster是否存在。验证Policy之间是否有Conflicts。如果两个drpolicy具有相同的cluster，同时有一个drpolicy是支持Metro DR的，那么这两个Policy会发生冲突。
...
1. 根据`ramenConfig.DrClusterOperator.DeploymentAutomationEnabled`配置，在受管集群上自动安装 DR Cluster controller。
2. 处理Fencing。
...
1. 从Spec.PlacementRef中获取User Placement Rule，placement rule 应该包含1个cluster replica并且scheduler是ramen
2. 从Spec.DRPolicyRef中获取DR Policy和包含的clusters
3. 如果PreferredCluster为空，则创建clone一个User Placement Rule
4. 根据Spec.Action执行：
  - 4.1 空值
    - 先从PreferredCluster或者clone的User Placement Rule获取home cluster
    - 在home cluster上创建primary的VRG，如果DR policy支持Metro则设置sync，如果支持Regional则设置Async，VGR的PVCSelector设置为spec.PVCSelector
  - 4.2 Failover
  - 4.3 RunRelocate
